...
function Withdraw() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.Fromaccount_name !== formData.Toaccount_name) {
            try {
                const response = await axios.post('http://127.0.0.1:5555/withdrawal', formData, {
                    headers: {
    ...
    return (
        <div className="flex justify-center items-center h-full">
            <Tooltip label="Withdraw Money" aria-label="withdraw Tooltip">
                <button onClick={onOpen} className="px-4 py-2 bg-green-500 text-white rounded-md shadow-md hover:bg-blue-600 transition-colors duration-300">
                    Withdraw
                </button>
            </Tooltip>
                    ...
                    <ModalBody className="p-6">
                        ...
                        <form className="space-y-4" onSubmit={handleSubmit}>
                            <div className="flex flex-col">
                                <label className="mb-2 font-medium" htmlFor="Fromaccount_name">From Account</label>
                                <select name="Fromaccount_name" className="p-2 border border-gray-300 rounded-md" onChange={handleChange} value={formData.Fromaccount_name} required>
                            ...
                            <div className="flex flex-col">
                                <label className="mb-2 font-medium" htmlFor="Toaccount_name">To Account</label>
                                <select name="Toaccount_name" className="p-2 border border-gray-300 rounded-md" onChange={handleChange} value={formData.Toaccount_name} required>
                                    <option value=''>Select Account</option>
                                    {filterAccount.map(account => (
                                        <option key={account.id} value={account.category}>{account.category}</option>
                                    ))}
